...
export default function SectionTestPage({ params }: { params: Promise<{ sectionName: string }> }) {
  const { sectionName } = use(params);
  ...
  const questions = useMemo<Question[]>(() => {
    ...
    // Aim for at least MIN_QUESTIONS; take all if fewer available
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-6 text-center pb-24 sidebar-offset">
  ...
  if (done) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 pb-24 text-center sidebar-offset" style={{ background: "#f0f4ff" }}>
  ...
  return (
    <div className="min-h-screen flex flex-col sidebar-offset pb-24" style={{ background: "#f0f4ff" }}>
      {/* Header */}
